...
threshold = 0.44 #subject-dependend
count = 0
paramaters_gaussian = {}
...
for i in range(len(input_list)):
    ...
    if(last_event == '{},{}'.format(input_list[i], output_list[i]) and
       input_list[i].startswith("Waiting")):
         continue
    ...
    if input_list[i] not in inputs_seen:
        ...
        alpha[input_list[i]] = {}
        alpha[input_list[i]]["a"] = np.ones(3, dtype="int64")
        alpha[input_list[i]]["b"] = np.ones(3, dtype="int64")
        #Bob:
        if(input_list[i]=="Switching power on,Pressing button to start,Waiting"):
            alpha[input_list[i]]["a"][0] += 20
            alpha[input_list[i]]["b"][0] += 20
            print(alpha[input_list[i]]["a"])
        if(input_list[i]=="Waiting"):
            alpha[input_list[i]]["a"][1] += 20
            alpha[input_list[i]]["b"][1] += 20
        if(input_list[i]=="Pressing button to start,Waiting"):
            alpha[input_list[i]]["a"][0] += 20
            alpha[input_list[i]]["b"][0] += 20
        if(input_list[i]=="Pressing button to stop"):
            alpha[input_list[i]]["a"][1] += 20
            alpha[input_list[i]]["b"][1] += 20
        ...
        if(input_list[i]=="Pressing button to start,Waiting"):
            ...
            alpha[input_list[i]]["b"][0] += 20
        if(input_list[i]=="Pressing button to stop"):
            alpha[input_list[i]]["a"][2] += 20
            alpha[input_list[i]]["b"][2] += 20
        if(input_list[i]=="Waiting"):
            alpha[input_list[i]]["a"][2] += 20
            alpha[input_list[i]]["b"][2] += 20"""
    ...
    #Compute the amount of surprise because of the time
    ...
    surprise = KL_dirichlet(a,b,index,sum_alpha)*a[index]
    ...
    # Categorization of surprise value (can be chosen individually)
    if surprise>(threshold+0.05):
            plot3.append("very surprising")
    elif surprise>(threshold+0.01):
    ...
    #Parameter updating for next iteration
...
# Only plot the first 10 occurrences
df_first_10 = df[df['Event_Index'] <= 10]
...
for i, cat in enumerate(df_first_10['plot2'].unique()):
    subset = df_first_10[df_first_10['plot2'] == cat]
...
plt.xticks(np.arange(len(df_first_10['plot2'].unique())) + 0.2, df_first_10['plot2'].unique(), rotation='vertical', fontsize=8)
plt.xlabel('Events', fontsize=12)
plt.ylabel('Count', fontsize=12)
plt.title('How surprising was each event in each of its first 10 occurrences', fontsize=14)
